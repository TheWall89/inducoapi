...
def test_get_employees_200_info(openapi_version):
    oapi = build_openapi(
        "GET", "/employees", "200", title="Custom Title", version="v1-custom", openapi_version=openapi_version
    )
    spec_dict, spec_url = read_from_filename(
...
def test_get_employees_200_response(openapi_version):
    with open("examples/employees.json") as f:
        response = f.read()
    oapi = build_openapi("GET", "/employees", "200", response=response, openapi_version=openapi_version)
    spec_dict, spec_url = read_from_filename(
...
def test_get_employees_200_response_ref(openapi_version):
    with open("examples/employees.json") as f:
        response = f.read()
    oapi = build_openapi(
        "GET", "/employees", "200", response=response, reference=True, openapi_version=openapi_version
    )
    spec_dict, spec_url = read_from_filename(
...
def test_get_employees_200_response_noexample(openapi_version):
    with open("examples/employees.json") as f:
        response = f.read()
    oapi = build_openapi(
        "GET", "/employees", "200", response=response, example=False, openapi_version=openapi_version
    )
    spec_dict, spec_url = read_from_filename(
...
def test_post_employees_201_request_response(openapi_version):
    ...
    with open("examples/new_employee_resp.json") as f:
        response = f.read()
    oapi = build_openapi(
        "POST", "/employees", "201", request=request, response=response, openapi_version=openapi_version
    )
    spec_dict, spec_url = read_from_filename(
...
def test_post_employees_201_request_invalid_json(openapi_version):
    ...
    with pytest.raises(ValueError) as excinfo:
        build_openapi("POST", "/employees", "201", request=request, openapi_version=openapi_version)
...
def test_post_employees_201_response_invalid_json(openapi_version):
    with open("tests/invalid.json") as f:
        response = f.read()
    with pytest.raises(ValueError) as excinfo:
        build_openapi("POST", "/employees", "201", response=response, openapi_version=openapi_version)
